...
function Workspace() {
    const { addTask, updateTask } = useContext(TaskContext); // Sử dụng updateTask từ context
    ...
    const handleFormSubmit = (taskData) => {
        if (editingTask) {
            updateTask(editingTask.id, taskData); // Gọi updateTask để cập nhật task
        } else {
            addTask(taskData); // Thêm task mới
        }
        setFormVisible(false); // Đóng form sau khi submit
    };
