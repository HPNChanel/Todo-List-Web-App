    ...
    return (
        <div className="workspace-container flex">
            ...
            <main className="workspace-main bg-gray-100 flex-grow p-4">
                ...
                <Routes>
                    <Route path="my-tasks" element={<TaskList tasks={getTasksByCategory('MY_DAY').filter(task => !task.completed)} onEdit={handleEditTask} />} />
                    <Route path="important" element={<TaskList tasks={getTasksByCategory('IMPORTANT').filter(task => !task.completed)} onEdit={handleEditTask} />} />
                    <Route path="planned" element={<TaskList tasks={getTasksByCategory('PLANNED').filter(task => !task.completed)} onEdit={handleEditTask} />} />
                    <Route path="completed" element={<TaskList tasks={getTasksByCategory('COMPLETED').filter(task => task.completed)} onEdit={handleEditTask} />} />
                </Routes>
            </main>
        </div>
    );
